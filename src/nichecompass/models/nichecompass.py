...
def _read_bool_mask_T(varm_value) -> np.ndarray:
    """
    Convert adata.varm[...] into a transposed, 2D numpy.bool_ array.
    Accepts scipy sparse matrices, numpy arrays, or pandas DataFrames.
    For an (n_genes x n_gps) input, returns (n_gps x n_genes).
    Works even when n_gps == 0 (empty / zero-column).
    """
    import numpy as _np
    import scipy.sparse as _sp
    import pandas as _pd

    if isinstance(varm_value, _pd.DataFrame):
        arr = varm_value.to_numpy()
    else:
        arr = varm_value

    # transpose then densify
    if _sp.issparse(arr):
        arr = arr.T.toarray()
    else:
        arr = _np.asarray(arr).T

    # ensure boolean (treat nonzeros as True)
    if arr.dtype != _np.bool_:
        arr = arr != 0

    # keep 2D even when empty
    if arr.ndim == 1:
        arr = arr.reshape(1, -1)
    return arr
...
class NicheCompass(BaseModelMixin):
    ...
    def __init__(self,
                 ...
                 seed: int=0,
                 omics_decoder_type: Literal["masked","fc"]="masked",
                 omics_decoder_n_layers: int = 1,
                 **kwargs):
        self.adata = adata
        ...
        self.seed_ = seed
        self.omics_decoder_type_ = omics_decoder_type
        self.omics_decoder_n_layers_ = omics_decoder_n_layers
        ...
        # In FC mode we allow running WITHOUT prior GP masks.
        if self.gp_targets_mask_key_ in adata.varm:
            targets_arr = _read_bool_mask_T(adata.varm[self.gp_targets_mask_key_])
            self.gp_targets_mask_ = torch.as_tensor(targets_arr, dtype=torch.bool)
        elif self.omics_decoder_type_ == "fc":
            # No prior GP masks → 0 prior GPs; all capacity will be in add-on GPs
            self.gp_targets_mask_ = torch.zeros((0, adata.n_vars), dtype=torch.bool)
        else:
            raise ValueError(
                "Please specify an adequate ´gp_targets_mask_key´ for your adata object. "
                "If you want to run without GP masks, set omics_decoder_type='fc'."
            )

        if self.gp_sources_mask_key_ in adata.varm:
            sources_arr = _read_bool_mask_T(adata.varm[self.gp_sources_mask_key_])
            self.gp_sources_mask_ = torch.as_tensor(sources_arr, dtype=torch.bool)
        elif self.omics_decoder_type_ == "fc":
            self.gp_sources_mask_ = torch.zeros((0, adata.n_vars), dtype=torch.bool)
        else:
            raise ValueError(
                "Please specify an adequate ´gp_sources_mask_key´ for your adata object. "
                "If you want to run without GP masks, set omics_decoder_type='fc'."
            )
            
        # Determine features scale factors
        if sp.issparse(self.adata.X):
            sums = np.asarray(self.adata.X.sum(axis=0)).ravel()
        else:
            sums = np.asarray(self.adata.X).sum(axis=0).ravel()
        sums = sums.astype(np.float32, copy=False)
        self.features_scale_factors_ = torch.from_numpy(
            np.concatenate([sums, sums], axis=0))
        ...
        use_fc = (self.omics_decoder_type_ == "fc")
        self.model = VGPGAE(
            ...
            use_fc_decoder=use_fc,
            fc_decoder_n_layers=self.omics_decoder_n_layers_,
    ...
    def train(self,
        ...
        if lambda_l1_masked > 0.:
            ...
            _targets_cats = self.adata.varm[self.gp_targets_categories_mask_key_]
            _sources_cats = self.adata.varm[self.gp_sources_categories_mask_key_]
            _targets_cats = (_targets_cats.toarray() if sp.issparse(_targets_cats) else np.asarray(_targets_cats))
            _sources_cats = (_sources_cats.toarray() if sp.issparse(_sources_cats) else np.asarray(_sources_cats))

            l1_targets_mask = torch.from_numpy(np.isin(_targets_cats, l1_targets_categories_encoded))
            l1_sources_mask = torch.from_numpy(np.isin(_sources_cats, l1_sources_categories_encoded))
        else:
        ...
        if ((len(self.cat_covariates_cats_) > 0) &
            ...
            for cat_covariates_embed_key, cat_covariate_embed in zip(
                ...
                self.get_cat_covariates_embeds()
                ):
    ...
    def get_recon_edge_probs(self,      
        ...
        compute_edge_thresh = (edge_thresh is None)

        # latent must already be stored
        if self.latent_key_ not in self.adata.obsm:
            raise ValueError(
                f"Please first store the latent representations in adata.obsm['{self.latent_key_}'] "
                "via model.get_latent_representation()."
            )

        # Cast to model dtype and rebuild full z (including inactive dims)
        param = next(self.model.parameters())
        dtype = param.dtype
        active_gp_mask = self.model.get_active_gp_mask()  # bool [n_gps_total]
        z_active = torch.as_tensor(self.adata.obsm[self.latent_key_],
                                   device=device, dtype=dtype)  # [n_obs, n_active]

        n_obs = z_active.shape[0]
        n_gps_total = active_gp_mask.shape[0]
        z_full = torch.zeros((n_obs, n_gps_total), device=device, dtype=dtype)
        active_idx = active_gp_mask.nonzero(as_tuple=True)[0]
        z_full.index_copy_(1, active_idx, z_active)
        ...
        if not return_edge_probs:
            ...
            for i in range(0, len(self.adata), node_batch_size):
                logits = self.model.graph_decoder(
                    z=z_full,
                    reduced_obs_start_idx=i,
                    reduced_obs_end_idx=i+node_batch_size
                )
                probs = torch.sigmoid(logits)  # [batch, n_nodes]
                ...
                if compute_edge_thresh:
                    if n_neighbors is None:
                        ...
                        n_neighs_adj = np.full((probs.shape[0],), n_neighbors, dtype=int)

                    probs_sorted = probs.sort(descending=True)[0]
                    edge_thresh_row = probs_sorted[
                        np.arange(probs_sorted.shape[0]), np.clip(n_neighs_adj-1, 0, probs_sorted.shape[1]-1)
                    ].view(-1, 1).expand_as(probs)
                    mask = (probs >= edge_thresh_row).long().cpu().numpy()
                else:
                    mask = (probs >= edge_thresh).long().cpu().numpy()

                adj_recon[i:i+node_batch_size, :] = mask
            ...
            logits = self.model.graph_decoder(z=z_full)
    ...
    def get_neighbor_importances(
        ...
        for i, node_batch in enumerate(node_loader):
            ...
            x_rna = node_batch.x[:, :self.n_output_genes_]
            _, alpha = self.model.rna_node_label_aggregator(
                x=x_rna, edge_index=node_batch.edge_index, return_agg_weights=True
            )

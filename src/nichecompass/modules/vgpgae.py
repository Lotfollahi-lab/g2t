...
class VGPGAE(nn.Module, BaseModuleMixin, VGAEModuleMixin):
    ...
    def __init__(self,
        ...
        self.use_fc_decoder_ = use_fc_decoder #new 
        ...
        self.node_label_aggregator = self.rna_node_label_aggregator
    ...
    def forward(self,
        ...
        if decoder == "omics":
            ...
            # Determine which features should be reconstructed based on static and
            # dynamic masks. In FC mode we treat the static masks as all-ones.
            # --- RNA (always present) ---
            if getattr(self, "use_fc_decoder_", False):
                dev = self.target_rna_dynamic_decoder_mask.device
                target_rna_decoder_static_mask = torch.ones(
                    (self.n_prior_gp_ + self.n_addon_gp_, self.n_output_genes_),
                    dtype=torch.bool, device=dev
                )
                source_rna_decoder_static_mask = torch.ones_like(target_rna_decoder_static_mask)
            else:
                if self.n_addon_gp_ > 0:
                    target_rna_decoder_static_mask = torch.cat(
                        (self.target_rna_decoder_mask, self.target_rna_decoder_addon_mask), dim=0
                    )
                    source_rna_decoder_static_mask = torch.cat(
                        (self.source_rna_decoder_mask, self.source_rna_decoder_addon_mask), dim=0
                    )
                else:
                    target_rna_decoder_static_mask = self.target_rna_decoder_mask
                    source_rna_decoder_static_mask = self.source_rna_decoder_mask

            # --- ATAC (only if present) ---
            if "atac" in self.modalities_:
                if getattr(self, "use_fc_decoder_", False):
                    # use RNA mask's device; ATAC dynamic mask may not exist in RNA-only runs
                    dev = self.target_rna_dynamic_decoder_mask.device
                    target_atac_decoder_static_mask = torch.ones(
                        (self.n_prior_gp_ + self.n_addon_gp_, self.n_output_peaks_),
                        dtype=torch.bool, device=dev
                    )
                    source_atac_decoder_static_mask = torch.ones_like(target_atac_decoder_static_mask)
                else:
                    if self.n_addon_gp_ > 0:
                        target_atac_decoder_static_mask = torch.cat(
                            (self.target_atac_decoder_mask, self.target_atac_decoder_addon_mask), dim=0
                        )
                        source_atac_decoder_static_mask = torch.cat(
                            (self.source_atac_decoder_mask, self.source_atac_decoder_addon_mask), dim=0
                        )
                    else:
            ...
            if "atac" in self.modalities_:
                ...
                if update_atac_dynamic_decoder_mask and not getattr(self, "use_fc_decoder_", False):
    ...
    def loss(self,
        ...
        # ---- mask-specific regularizers: only if not using FC decoders ----
        using_fc = getattr(self, "use_fc_decoder_", False)
        
        if not using_fc:
            loss_dict["masked_gp_l1_reg_loss"] = (lambda_l1_masked *
                compute_gp_l1_reg_loss(self, gp_type="prior",
            ...
            if self.n_addon_gp_ != 0:
                loss_dict["addon_gp_l1_reg_loss"] = (lambda_l1_addon *
                    compute_gp_l1_reg_loss(self, gp_type="addon"))
        else:
            # when FC, define zeros so keys exist
            dev = self.target_rna_theta.device  # any model param device
            zero = torch.tensor(0.0, device=dev)
            loss_dict["masked_gp_l1_reg_loss"] = zero
            loss_dict["group_lasso_reg_loss"]   = zero
            loss_dict["addon_gp_l1_reg_loss"]   = zero
        ...
        if self.include_cat_covariates_contrastive_loss_ and \
        ("cat_covariates_contrastive_loss" in loss_dict):
        ...
        if self.include_gene_expr_recon_loss_:
            ...
            # add reg terms only in masked mode
            if not using_fc:
        ...
        if ("atac" in self.modalities_) and self.include_chrom_access_recon_loss_:
    ...
    @torch.no_grad()
    def get_gp_weights(self, only_masked_features: bool=False, gp_type: Literal["all", "prior", "addon"]="all") -> List[torch.Tensor]:
        """
        ...
        """
        # FC decoders don’t have masked/addon weight matrices; this utility
        # is only meaningful for masked decoders.
        # FC decoders don't have masked/addon linear layers; return a placeholder
        if getattr(self, "use_fc_decoder_", False):
            dev = next(self.parameters()).device
            n_gps = self.n_prior_gp_ + self.n_addon_gp_
            rna_dummy  = torch.zeros(2 * self.n_output_genes_, n_gps, device=dev)
            if "atac" in self.modalities_:
                atac_dummy = torch.zeros(2 * self.n_output_peaks_, n_gps, device=dev)
                return [rna_dummy, atac_dummy]
            return [rna_dummy]
        
        # --- existing masked-decoder code below ---
        gp_weights_all_modalities = []
        for modality in self.modalities_:
    ...
    def get_active_gp_mask(
        self,
        abs_gp_weights_agg_mode: Literal["sum", "nzmeans", "sum+nzmeans", "nzmedians", "sum+nzmedians"]="sum+nzmeans",
        return_gp_weights: bool=False,
        normalize_gp_weights_with_features_scale_factors: bool=False,
        ) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        """
        Determine active gene programs from RNA decoder weights.
        In FC mode, all GPs are active and there are no decoder weights to inspect.
        """
        # FC decoders: everything active; no weights to inspect
        if getattr(self, "use_fc_decoder_", False):
            device = next(self.parameters()).device
            mask = torch.ones(self.n_prior_gp_ + self.n_addon_gp_, dtype=torch.bool, device=device)
            if return_gp_weights:
                return mask, None
            return mask

        device = next(self.parameters()).device
        active_gp_mask = torch.zeros(self.n_prior_gp_ + self.n_addon_gp_, dtype=torch.bool, device=device)
        ...
        for gp_type in gp_types:
            gp_weights = self.get_gp_weights(only_masked_features=False, gp_type=gp_type)[0]
            if gp_weights.shape[1] == 0:
                continue
            ...
            # normalize with feature scale factors if requested
            if normalize_gp_weights_with_features_scale_factors:
                gp_weights_norm = gp_weights / self.features_scale_factors_[:, None].to(device)
            else:
                gp_weights_norm = gp_weights

            # weight by running mean abs mu
            gp_weights_norm = self.running_mean_abs_mu[gp_idx] * gp_weights_norm

            # base summaries
            abs_sum = gp_weights_norm.abs().sum(dim=0)  # L1 sum over features
            if abs_gp_weights_agg_mode in ["sum", "sum+nzmeans", "sum+nzmedians"]:
                thresh = self.active_gp_thresh_ratio_ * abs_sum.max()
                active_gp_mask[gp_idx] |= (abs_sum >= thresh)

            if abs_gp_weights_agg_mode in ["nzmeans", "sum+nzmeans"]:
                counts = torch.count_nonzero(gp_weights_norm, dim=0)
                nzmean = torch.nan_to_num(abs_sum / counts)
                thresh = self.active_gp_thresh_ratio_ * nzmean.max()
                active_gp_mask[gp_idx] |= (nzmean >= thresh)

            if abs_gp_weights_agg_mode in ["nzmedians", "sum+nzmedians"]:
                zero_mask = (gp_weights_norm == 0)
                nan_t = torch.tensor(float("nan"), device=gp_weights_norm.device, dtype=gp_weights_norm.dtype)
                with_nan = torch.where(zero_mask, nan_t, gp_weights_norm.abs())
                nzmedian = torch.nanmedian(with_nan, dim=0).values
                nzmedian = torch.nan_to_num(nzmedian)
                thresh = self.active_gp_thresh_ratio_ * nzmedian.max()
                active_gp_mask[gp_idx] |= (nzmedian >= thresh)

        if return_gp_weights:
            # Return weights for *all* GPs, filtered by active mask
            all_w = self.get_gp_weights(only_masked_features=False, gp_type="all")[0]
            return active_gp_mask, all_w[:, active_gp_mask]
    ...
    @torch.no_grad()
    def get_omics_decoder_outputs(self, node_batch: Data, only_active_gps: bool=True) -> dict:
        """
        Thin wrapper so callers don't have to pass decoder='omics'.
        Returns NB mean tensors per modality/entity with FULL feature width.
        """
        out = self.forward(
            data_batch=node_batch,
            decoder="omics",
            use_only_active_gps=only_active_gps,
            return_agg_weights=False
        )

        B = node_batch.batch_size
        device = out["target_rna_nb_means"].device
        full = {}

        # RNA: expand to full n_genes
        full["target_rna_nb_means"] = torch.zeros(B, self.n_output_genes_, device=device)
        full["source_rna_nb_means"] = torch.zeros(B, self.n_output_genes_, device=device)
        trg_idx = self.features_idx_dict_["target_reconstructed_rna_idx"]
        src_idx = self.features_idx_dict_["source_reconstructed_rna_idx"]
        full["target_rna_nb_means"][:, trg_idx] = out["target_rna_nb_means"]
        full["source_rna_nb_means"][:, src_idx] = out["source_rna_nb_means"]

        # ATAC (if present): expand to full n_peaks
        if "atac" in self.modalities_:
            full["target_atac_nb_means"] = torch.zeros(B, self.n_output_peaks_, device=device)
            full["source_atac_nb_means"] = torch.zeros(B, self.n_output_peaks_, device=device)
            trg_p_idx = self.features_idx_dict_["target_reconstructed_atac_idx"]
            src_p_idx = self.features_idx_dict_["source_reconstructed_atac_idx"]
            full["target_atac_nb_means"][:, trg_p_idx] = out["target_atac_nb_means"]
            full["source_atac_nb_means"][:, src_p_idx] = out["source_atac_nb_means"]
